	...
	for P in [ 2, 4, 8, 12, 16, 20, 24, 32, 40, 48, 56, 64]:
		...
		for s in s_values:
			...
			if s > 1 and s <= P:
				...
				# with load balancing
				command1 = f'qsub -l nodes={nodes}:ppn={ppn} -v afile="{afile}",pfile="{pfile}",lb="1",s={s} pbs_readings.sh' 
				print(command1)
				os.system(command1)
				...
				command2 = f'qsub -l nodes={nodes}:ppn={ppn} -v afile="{afile}",pfile="{pfile}",lb="0",s={s} pbs_readings.sh' 
				print(command2)
				os.system(command2)
