...
vector<int> loadbalancing(int totalElements, int totalProcessors, vector<int>& arr, int low, int high, int pid) {
    ...
    while (docontinue > 0)
    {
        ...
        */
        ...
        printf("\nMYID %d reached Barrier 2; counter = %d, localarray = %d", pid, counter, localArray.size());
        MPI_Barrier(MPI_COMM_WORLD);
    }
    //MPI_Barrier(MPI_COMM_WORLD);
    printf("\nStarting right to left load balancing. LocalArray = %d\n", localArray.size());
    ...
    while (docontinue > 0)
    {
        ...
        printf("\nMYID %d reached Barrier 4; counter = %d, localarray = %d", pid, counter, localArray.size());
        MPI_Barrier(MPI_COMM_WORLD);
    }
    printf("\nFinished LB. MYID = %d, localArray = %d\n", pid, localArray.size());
    return localArray;
}
...
int main(int argc, char** argv) {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    data = loadbalancing(data.size(), P, data, low, high, myid);
    int token = 566;
